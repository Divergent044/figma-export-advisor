...
export function renderFormatButtons(): void {
	...
	buttons.forEach((btn: Element) => {
		...
		btn.classList.toggle(
			...
			fmt === recommendedFormat && fmt !== selectedFormat,
		);
	});
...
export function renderDensityButtons(): void {
	...
	buttons.forEach((btn: Element) => {
		...
		const isActive =
			btnConstraint.type === selectedConstraint.type &&
			btnConstraint.value === selectedConstraint.value;
		btn.classList.toggle('active', isActive);
		btn.setAttribute('aria-checked', isActive ? 'true' : 'false');
	});
}
...
export function renderAnalysisCard(): void {
	...
	const transparencyCount = analyses.filter(
		(a: UINodeAnalysis) => a.hasTransparency,
	).length;
	...
	function createRow(
		...
		valueSpan.className =
			'analysis-value' + (valueClass ? ' ' + valueClass : '');
	...
	analysisCard.appendChild(
		createRow(t('ui.row_content_type'), t(contentTypeKey)),
	);
	analysisCard.appendChild(
		createRow(t('ui.row_recommended'), recommendedFormat, 'recommended'),
	);
