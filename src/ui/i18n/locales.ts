export const LOCALES: Record<string, Record<string, string>> = {
  en: {
    ...
    'ui.png_confirm_text1': 'You selected PNG for content containing raster images. The plugin cannot automatically detect transparency in such images.',
    'ui.png_confirm_text2': 'Are you sure these images need transparency for use in layout?',
    'ui.png_confirm_text3': 'If transparency is not needed, JPG will provide a smaller file size without quality loss.',
    'ui.png_confirm_btn': 'Confirm PNG',
    'ui.png_switch_jpg': 'Switch to JPG',
    'ui.optimize_title': 'Optimize Your Image',
    'ui.optimize_text': 'File exported successfully! We recommend optimizing it to reduce file size without visible quality loss.',
    ...
    'desc.mixed_content_svg': 'Selection contains mixed content (vector + raster). SVG does not support raster elements. PNG is recommended.',
    'desc.mixed_content_jpg': 'Selection contains mixed content (vector + raster). JPG does not support vector elements. PNG is recommended.',
    'desc.mixed_content_png': 'Selection contains mixed content (vector + raster). PNG is recommended as a universal format for both types.',
    'desc.vector_not_svg': 'Content is vector graphics. SVG is recommended to preserve scalability without quality loss.',
    'desc.raster_transparency_jpg': 'JPG does not support transparency. Effects requiring alpha channel detected. PNG is recommended.',
    'desc.raster_transparency_svg': 'SVG is not optimal for raster content with transparency. PNG is recommended.',
    'desc.raster_image_svg': 'SVG is not suitable for raster images. JPG is recommended (or PNG if images contain transparency).',
    'desc.raster_no_transparency_png': 'Content has no transparency or alpha-channel effects. JPG will provide a smaller file size without quality loss.',
    'desc.raster_no_transparency_svg': 'SVG is not optimal for raster content without transparency. JPG is recommended for a smaller file size.',
  ...
  ru: {
    ...
    'ui.png_confirm_text1': 'Вы выбрали формат PNG для контента, содержащего растровые изображения. Плагин не может автоматически определить наличие прозрачности в таких изображениях.',
    'ui.png_confirm_text2': 'Уверены, что изображениям необходима прозрачность для использования в вёрстке?',
    'ui.png_confirm_text3': 'Если прозрачность не нужна — JPG обеспечит меньший размер файла без потери качества.',
    'ui.png_confirm_btn': 'Подтвердить PNG',
    'ui.png_switch_jpg': 'Переключить на JPG',
    'ui.optimize_title': 'Оптимизируйте изображение',
    'ui.optimize_text': 'Файл успешно экспортирован! Рекомендуем оптимизировать его для уменьшения размера без видимой потери качества.',
    ...
    'desc.mixed_content_svg': 'Выделение содержит смешанный контент (вектор + растр). SVG не подходит для растровых элементов. Рекомендуется PNG.',
    'desc.mixed_content_jpg': 'Выделение содержит смешанный контент (вектор + растр). JPG не подходит для векторных элементов. Рекомендуется PNG.',
    'desc.mixed_content_png': 'Выделение содержит смешанный контент (вектор + растр). Рекомендуется PNG как универсальный формат для обоих типов.',
    'desc.vector_not_svg': 'Содержимое является векторной графикой. Рекомендуется SVG для сохранения масштабируемости без потери качества.',
    'desc.raster_transparency_jpg': 'JPG не поддерживает прозрачность. Обнаружены эффекты, требующие альфа-канала. Рекомендуется PNG.',
    'desc.raster_transparency_svg': 'SVG не оптимален для растрового контента с прозрачностью. Рекомендуется PNG.',
    'desc.raster_image_svg': 'SVG не подходит для растровых изображений. Рекомендуется JPG (или PNG, если изображения содержат прозрачность).',
    'desc.raster_no_transparency_png': 'Контент не содержит прозрачности или эффектов с альфа-каналом. JPG обеспечит меньший размер файла без потери качества.',
    'desc.raster_no_transparency_svg': 'SVG не оптимален для растрового контента без прозрачности. Рекомендуется JPG для меньшего размера файла.',
    'desc.generic_mismatch': 'Выбранный формат не является оптимальным для данного контента.',
  },
};
